...
class TimezonePage(Pager):
	...
	def get_zonearea_tup(self,forfind=False):
		tup = ('GMT+14:00','GMT+13:00','GMT+12:45','GMT+12:00','GMT+11:30','GMT+11:00','GMT+10:30','GMT+10:00','GMT+09:30','GMT+09:00','GMT+08:45','GMT+08:00','GMT+07:00','GMT+06:30','GMT+06:00','GMT+05:30','GMT+05:00','GMT+04:30','GMT+04:00','GMT+03:30','GMT+03:00','GMT+02:00','GMT+01:00','GMT+00:00','GMT-01:00','GMT-02:00','GMT-03:00','GMT-03:30','GMT-04:00','GMT-04:30','GMT-05:00','GMT-06:00','GMT-07:00','GMT-07:30','GMT-08:00','GMT-08:30','GMT-09:00','GMT-09:30','GMT-10:00','GMT-10:30','GMT-11:00','GMT-12:00')
		if forfind:
			if forfind not in tup:return -1
	...
	def get_weekseq_tup(self,forfind=False):
		tup = ('First','Second','Third','Forth','Last')
		if forfind:
			if forfind not in tup:return -1
			return tup.index(forfind)+1
		return tup
	...
	def create_grid(self):
		for x in range(self.row):
			for y in range(self.column):
				fm = tk.Frame(self.frame,width=self.width/self.column-1,height=self.height/self.row-1)
				fm.grid(row=x,column=y,padx=1) #,pady=1,ipadx=6)
	...
	def check_limit_dependence(self,key,value):
		if key not in ['SDAY','EDAY','SMONTH','EMONTH']:return True
		if key == 'SDAY':
			if 'SMONTH' not in self.opts_dict:return True
			if int(value) < 28:return True
			month = int(self.opts_dict['SMONTH'])
			if month == 2 and int(value) > 28:
				# CommonFunc.windows_info('开始月份为2月时，日期不能设置大于28号！','warn')
				self.reset_combox_list([[1,1]])
				self.pop_opts_dict('SDAY')
				return False
			elif month in [4,6,9,11] and int(value) > 30:
				# CommonFunc.windows_info('开始月份为%s月时，日期不能设置大于30号！'%(month),'warn')
				self.reset_combox_list([[1,1]])
				self.pop_opts_dict('SDAY')
				return False
		elif key == 'EDAY':
			if 'EMONTH' not in self.opts_dict:return True
			if int(value) < 28:return True
			month = int(self.opts_dict['EMONTH'])
			if month == 2 and int(value) > 28:
				# CommonFunc.windows_info('结束月份为2月时，日期不能设置大于28号！','warn')
				self.reset_combox_list([[1,2]])
				self.pop_opts_dict('EDAY')
				return False
			elif month in [4,6,9,11] and int(value) > 30:
				# CommonFunc.windows_info('结束月份为%s月时，日期不能设置大于30号！'%(month),'warn')
				self.reset_combox_list([[1,2]])
				self.pop_opts_dict('EDAY')
				return False
		elif key == 'SMONTH':
			if 'SDAY' not in self.opts_dict:return True
			...
			if int(value) == 2 and day > 28:
				...
				# CommonFunc.windows_info('开始月份为%s月时，日期不能设置大于30号！'%(value),'warn')
				self.reset_combox_list([[0,1]])
				self.pop_opts_dict('SMONTH')
				return False
		elif key == 'EMONTH':
			if 'EDAY' not in self.opts_dict:return True
